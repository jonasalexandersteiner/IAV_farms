...
sample_pigs_path <- "C:/Users/js22i117/OneDrive - Universitaet Bern/Dokumente/SIV_Projekt/Data_processing/SIV_farms/01_oridata/Samples_pigs_qPCR.xlsx"
...
df2 <- df1 %>%
  ...
  mutate(
    ...
    seperation_quarantine_area = as.factor(seperation_quarantine_area),
  ...
  rename(
    ...
    farrowing_room_temperature = farrowing_room_temperature,
    weaners_room_temperature = weaners_room_temperature,
  ...
  relocate(
    total_samples, positive_pigs, percent_positive_pigs,
    min_ct, max_ct, mean_ct, sd_ct, symptomatic_report,
    .after = SIV_positive
  ) %>%
  relocate(
...
df3 <- df2 %>%
  ...
  # 2. Mutations and transformations (see domain documentation for details)
   ...
   mutate(
    ...
    seperation_quarantine_area = factor(
      case_when(
        seperation_quarantine_area == 2 ~ "1",
        seperation_quarantine_area == 3 ~ "2",
        seperation_quarantine_area %in% c(1, 4) ~ "3",
        TRUE ~ NA_character_
      ),
      levels = c("1", "2", "3"),
    ),
  ...
  dplyr::select(
    -any_of(c(
      ...
      "gestation_sows_reduced_general_wellbeing", "gestation_sows_sneezing", "gestation_sows_coughing", "gestation_sows_discharge","sgd_qgs",
...
#   - Pie charts for SIV_positive, symptomatic_report, ili_signs (TRUE vs FALSE)
...
# =====================================================================
# BOX PLOT WITH MEAN LINE (for percent_positive_pigs)
# =====================================================================
save_beeswarm_boxplot <- function(df, var_y, group_var, filename) {
  df_plot <- df %>%
    filter(!is.na(.data[[var_y]]), .data[[var_y]] > 0, !is.na(.data[[group_var]])) %>%
    mutate(!!group_var := factor(.data[[group_var]], levels = c(FALSE, TRUE)))
  
  gg <- ggplot(df_plot, aes(x = .data[[group_var]], y = .data[[var_y]], fill = .data[[group_var]])) +
    stat_summary(fun = mean, geom = "errorbar",
                 aes(ymax = ..y.., ymin = ..y..),
                 width = 0.4, color = "black", size = 1.5) +
    stat_summary(fun = mean, geom = "text",
                 aes(label = sprintf("%.1f", ..y..)),
                 vjust = -0.6, size = 5, color = "black") +
    geom_boxplot(alpha = 0.7, outlier.color = NA) +
    geom_beeswarm(color = "black", size = 2) +
    scale_fill_manual(values = cb_colors) +
    scale_x_discrete(labels = c("No", "Yes")) +
    labs(x = "Symptomatic report", y = "Percent of positive pigs") +
    coord_cartesian(ylim = c(0, 100)) +
    theme_pub +
    theme(legend.position = "none")
  
  ggsave(filename, gg, width = 7, height = 7, device = "svg")
}
...
# PIE CHARTS
save_pie_chart(df3, "SIV_positive", file.path(output_folder, "pie_SIV_positive.svg"))
save_pie_chart(df3, "symptomatic_report", file.path(output_folder, "pie_symptomatic_report.svg"))
save_pie_chart(df3, "ili_signs", file.path(output_folder, "pie_ili_signs.svg"))
...
# BOX PLOT: mean Ct by symptomatic report
save_box_plot(df3, "mean_ct", "symptomatic_report", file.path(output_folder, "box_mean_ct_by_symptomatic_report.svg"))

# BOX PLOT: number of weaners by SIV status
save_box_plot(df3, "number_weaners", "SIV_positive", file.path(output_folder, "box_number_weaners_by_SIV_status.svg"))

# PROPORTION BAR PLOT: SIV status by bird contact
save_prop_barplot(df3, "contact_bird_in_stable", "SIV_positive",
...
save_prop_barplot(df2, "respiratory_history_human", "SIV_positive",
                  file.path(output_folder, "prop_SIV_by_respiratory_history_human.svg"),
                  x_label = "Respiratory history (human)")

# BOX + BEESWARM: percent positive pigs by symptomatic report
save_beeswarm_boxplot(df3, "percent_positive_pigs", "symptomatic_report",
                      file.path(output_folder, "beeswarm_percent_positive_by_symptomatic_report.svg"))
